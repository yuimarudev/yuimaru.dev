...
const config = {
  preprocess: [preprocess(), ioniconsImportOptimizer(), optimizeCarbonImports()],
  kit: {
    adapter: adapter()
  }
};
      ...
      const code = walkAndReplace(
        ...
        ({ node }, replaceContent) => {
          if (node.type === "ImportDeclaration") {
            if (node.source.value === "svelte-awesome-icons") {
              replaceContent(
                node,
                node.specifiers
                  .map(({ local }) => {
                    return `import ${local.name} from "svelte-awesome-icons/${local.name}.svelte"`;
                  })
                  .join("\n")
              );
            }
